"""
CTA Exhaustion Signal Generator
...
# Set EMA windows based on CTA type
if CTA_TYPE == "slow":
    SHORT_WINDOW = 50
    MID_WINDOW = 100
    LONG_WINDOW = 200
...
# Process currencies: invert and convert to USD base
inverse = ["EUR", "GBP", "AUD", "NZD"]
df_fx[inverse] = 1 / df_fx[inverse]

# Convert European currencies to EUR-based rates
euroy = ["GBP", "SEK", "NOK", "HUF", "PLN", "CZK"]
df_fx[euroy] = df_fx[euroy].multiply(df_fx["EUR"], axis=0)

print("Calculating CTA positioning for all currencies...")
...
for currency in df_fx.columns:
    ...
    if len(price_series) < LONG_WINDOW:
        print(f"Skipping {currency}: insufficient data")
        continue
...
# Calculate exhaustion signal thresholds
print("Calculating exhaustion signal thresholds...")
...
thresholds = {
    cur: (
        upper,
        upper - GAP if upper - GAP > 0 else upper - 3,
        lower,
        lower + GAP if lower + GAP < 0 else lower + 3
    )
    for cur, upper, lower in zip(percentiles_90.index, percentiles_90, percentiles_10)
}

# Generate exhaustion signals
print("Generating exhaustion signals...")

def generate_directional_exhaustion_signals(positions_df, thresholds):
...
signals_df = generate_directional_exhaustion_signals(positions_df, thresholds)
signal_counts = (signals_df == 1).sum()
print(f"Generated signals for {len(signal_counts[signal_counts > 0])} currencies")
...
# Generate charts
def create_exhaustion_model(df, ccy, currency):
    fig = go.Figure()
    ...
    fig.update_layout(
        title=dict(
            text=f"{ccy} - CTA Positioning & Exhaustion Signals",
            x=0.5,
            xanchor='center',
...
for ccy in df_display.columns:
    ...
    if currency not in positions_df.columns:
        continue
    try:
        fig = create_exhaustion_model(df_display, ccy, currency)
        filename = os.path.join(OUTPUT_DIR, f"{ccy}_exhaustion.html")
...
print(latest_positions.head())
print("\nTop 5 Short Positions:")
print(latest_positions.tail())

# Save summary data
summary = {
    'generated_at': datetime.now().isoformat(),
    'cta_type': CTA_TYPE,
    'windows': f"{SHORT_WINDOW}/{MID_WINDOW}/{LONG_WINDOW}",
...
print(f"\n✅ Complete! Generated {chart_count} charts in {OUTPUT_DIR}/")
